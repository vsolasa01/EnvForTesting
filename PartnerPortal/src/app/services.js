(function () {
    ...
    function sessionService($q, $log, $http, $state, $timeout, $window, $location, localStorageService, snapflowConstant, Login, managedModal, geoCodeService, Partner, $rootScope) {
        /* jshint validthis:true */
        var self = this,
        ...
        function sessionHasExpired() {
            ...
            //expiryTime = moment(loginTimeStamp.timeStamp).add(600, 'minutes');
            currentTime = moment();
                ...
                .then(function (data) {
                    ...
                    if(data.profile && data.profile.Role && data.profile.Role !== null && typeof data.profile.Role !== 'undefined'){
                        ...
                        if(temp !== null && temp!== '' && temp.length === 0){
                            ...
                            if(data.profile.Role.split(',')[0] === 'PartnerUser' || data.profile.Role.split(',')[0] === 'Partner User'){
                                localStorageService.add('userCurrentRole', 'SalesPerson');
                            }else if(data.profile.Role.split(',')[0] === 'OlympusManager' || data.profile.Role.split(',')[0] === 'Olympus Manager'){
                                localStorageService.add('userCurrentRole', 'OlympusManager');
                            }else if(data.profile.Role.split(',')[0] === 'SunEdisonManager' || data.profile.Role.split(',')[0] === 'SunEdison Manager'){
                                localStorageService.add('userCurrentRole', 'SalesManager');
                            }else{
                    ...
                    /*localStorageService.add('userCurrentRole', 'SalesManager');
                    if(localStorageService.get('userCurrentRole') !== 'SalesManager'){
                        localStorageService.add('se-chosenPartner', data.profile.PartnerId);
                    }else{
		...
		localStorageService.remove('proposalID');
                ...
                localStorageService.remove('se-role');
		localStorageService.remove('partnerList');
                localStorageService.remove('ProposalTool');
                localStorageService.remove('se-chosenPartner');
                localStorageService.remove('se-chosenPartnerName');
        ...
        function setSessionTimeout() {
            ...
            function setTimeoutDuration(timeStamp) {
                ...
                expiryTime = moment(timeStamp).add(600, 'minutes');
                currentTime = moment();
    ...
    function homeownerService($q, sessionService, $rootScope, Homeowner, $log, $timeout, asAddressFilter, geoCodeService, homeownerModalService, localStorageService, $window,CreditCheckPoll) {
        ...
        function retrieveHomeownersByPartnerID() {
            ...
            viewModelsPromise = Homeowner.get({partnerid: localStorageService.get('se-chosenPartner'), LastUpdatedOn: lastUpdatedOn}).$promise
                .then(function (data) {
        ...
        function retrieveHomeownersBySalesPersonID() {
            ...
            viewModelsPromise = Homeowner.get({SalesPersonId: sessionService.user.profile.NSInternalId, LastUpdatedOn: lastUpdatedOn}).$promise
                .then(function (data) {
                    if(typeof data.TotalCount !== 'undefined' && data.TotalCount){
                        var totalNoOfLeads = parseInt(data.TotalCount);
                        if(totalNoOfLeads > 0 && (typeof tempPageInfo.totalNoOfPages === 'undefined' || tempPageInfo.totalNoOfPages === null)){
        ...
        function retrieveHomeownersBySunEdisonManagerID() {
            ...
            viewModelsPromise = Homeowner.get({SunEdisonMgrId: sessionService.user.profile.NSInternalId, LastUpdatedOn: lastUpdatedOn}).$promise
                .then(function (data) {
                    if(typeof data.TotalCount !== 'undefined' && data.TotalCount){
                        var totalNoOfLeads = parseInt(data.TotalCount);
                        if(totalNoOfLeads > 0 && (typeof tempPageInfo.totalNoOfPages === 'undefined' || tempPageInfo.totalNoOfPages === null)){
        ...
        function pollHomeownerUntilCreditStatusExists(homeownerViewModel, triesRemaining) {
            ...
            return CreditCheckPoll.getNoCache({'id': homeownerViewModel.model.SunEdCustId}).$promise.then(function (data) {
                ...
                if (data.CreditStatus) {
                    ...
                    homeownerViewModel.model.CreditStatus = data.CreditStatus;
        ...
        function populateFinancingProgram(homeownerVM) {
            switch (homeownerVM.model.PurchaseType) {
                ...
                case 'PPA':
                    ...
                    break;
                case 'Loan Mosaic':
                    homeownerVM.model.FinancingProgram = 'SunEdison Mosaic SCION';
                    break;
                case 'Loan Mosaic Signature':
                    homeownerVM.model.FinancingProgram = 'SunEdison Mosaic SCION With Signature Series';
                    break;
            }
        }
        ...
        function updateFrom(originalVM, updatedVM) {
            ...
            if(vmToSubmit.model.PurchaseType === 'Loan Mosaic' || vmToSubmit.model.PurchaseType === 'Loan Mosaic Signature'){
                vmToSubmit.model.PurchaseType = 'Loan';
            }
   ...
   function proposalIdService(ProposalIDfact,ContractIdfact) {
    ...
    angular
        ...
        .service('sessionService', ['$q', '$log', '$http', '$state', '$timeout', '$window', '$location', 'localStorageService', 'snapflowConstant', 'Login', 'managedModal', 'geoCodeService', 'Partner', '$rootScope', sessionService])
